...
object_detec = cv2.createBackgroundSubtractorMOG2(varThreshold=100)
...
while True:
    ...
    conture, h = cv2.findContours(mask_roi, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for cnt in conture:
        ...
        if area > 200:
            ...
            dat = cnt
    """
    cv2.line(frame, (80, count_line_pos), (600, count_line_pos), (0, 0, 255), 3)
    cv2.line(frame, (600, count_line_pos), (1200, count_line_pos), (255, 0, 0), 3)

    for (i, c) in enumerate(conture):
        ...
        cv2.putText(frame, "VEHICLE COUNT (in): " + str(couter), (x, y -20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        #print("again")
        ...
        for (x, y) in detect:
            ...
            if y < (count_line_pos + offset) and y > (count_line_pos - offset) and (x < 600):

                couter += 1
                cv2.line(frame, (80, count_line_pos), (600, count_line_pos), (0, 255, 0), 5)

            if y < (count_line_pos + offset) and y > (count_line_pos - offset) and (x > 600):
    
                outgo += 1
                cv2.line(frame, (600, count_line_pos), (1200, count_line_pos), (0, 255, 0), 5)

            detect.remove((x, y))
            #print("VAHICLE COUNT" + str(couter))

    cv2.putText(frame, "VEHICLE COUNT (in): " + str(couter), (250, 70), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 5)
    cv2.putText(frame, "VEHICLE COUNT (out) : " + str(outgo), (250, 200), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 5)
